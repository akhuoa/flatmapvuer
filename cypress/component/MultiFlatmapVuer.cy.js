        ...
        cy.get('.flatmapvuer-popover')
          ...
          .then(() => {
            ...
            cy.get('.checkall-display-text').then(($label) => {
              ...
              cy.get('.pathway-location > .pathway-container:visible')
                .contains('Notes')
                .parent()
                .siblings()
              ...
              cy.get('@canvas').screenshot(
                'base/cypress/component/MultiFlatmapVuer.cy.js/mapalert',
              );
              // UI
              cy.get('@canvas').screenshot(
                'MultiFlatmapVuer.cy.js/base/cypress/component/MultiFlatmapVuer.cy.js/mapalert',
              );
              ...
              .then(() => {
                ...
                cy.get('#tooltip-container')
                  ...
                  .then(() => {
                    ...
                    cy.get('.subtitle')
                      .should('exist')
                      .contains('Studied in Rattus norvegicus species');

                    // Open the 'show more' section
                    cy.get('#show-path-info').should('exist').click();

                    cy.get(
                      '[origin-item-label="lamina VII of gray matter of spinal cord, L6 segment of lumbar spinal cord"]',
                    ).should('exist');
                    ...
                    cy.get('.citation-list li.loading')
                      .should('not.exist')
                      .then(() => {
            ...
            cy.then(() => {
              ...
              organTypes.forEach((type) => {
                cy.then(() => {
                  ...
                  // CLI
                  cy.get('@canvas').screenshot(
                    `base/cypress/component/MultiFlatmapVuer.cy.js/${species.name}_${type}`,
                  );
                  // UI
                  cy.get('@canvas').screenshot(
                    `MultiFlatmapVuer.cy.js/base/cypress/component/MultiFlatmapVuer.cy.js/${species.name}_${type}`,
                  );
                });
              });
            });
        ...
        cy.get('@readySpy')
          ...
          .then(() => {
            ...
            for (let i = 0; i < organTypes.length; i++) {
              ...
              cy.get('@canvas')
                .compareSnapshot(`${species.name}_${type}`)
                .then((comparisonResults) => {
                  ...
                  expect(
                    ...
                    `${species.name}_color maps should be almost identical`,
